...
// InternshipCard functional component
// Props are directly destructured from the argument object
const InternshipCard = ({ internship, onEdit, onDelete }) => {

  // Helper function to determine status badge color based on status string
  const getStatusColor = (status) => {
    // Convert status to lowercase for robust comparison
    const lowerCaseStatus = status.toLowerCase();
    return lowerCaseStatus === 'open' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
  };

  return (
    // Main card container with shadow on hover effect
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden border border-gray-200">
      <div className="p-6">
        {/* Header section: Title, Department, and Status Badge */}
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{internship.title}</h3>
            <div className="flex items-center text-sm text-gray-500 mb-2">
              <MapPin className="h-4 w-4 mr-1" /> {/* Location/Department icon */}
              {internship.department}
            </div>
          </div>
          {/* Status badge, dynamically colored based on status */}
        ...
        {/* Internship Details: Duration, Stipend, Applicants */}
        <div className="space-y-2 mb-4">
          ...
          <div className="flex items-center text-sm text-gray-500">
            <Banknote className="h-4 w-4 mr-2" /> {/* Dollar sign icon */}
            {internship.stipend}/month
          </div>
          <div className="flex items-center text-sm text-gray-500">
            <Users className="h-4 w-4 mr-2" /> {/* Users icon */}
            {internship.applicants} applicants
          </div>
        </div>

        {/* Footer section: Posted Date and Action Buttons (Edit/Delete) */}
        <div className="flex items-center justify-between pt-4 border-t border-gray-200">
          ...
          <div className="flex space-x-2">
            {/* Edit Button */}
            <button
              onClick={() => onEdit(internship)} // Calls onEdit with the current internship object
            ...
            {/* Delete Button */}
            <button
              onClick={() => onDelete(internship.id)} // Calls onDelete with the internship ID
